...
export interface RecycleBulkCSVUploadProps {
  onUpload: (file: File) => Promise<void>
  uploading?: boolean
  error?: string
  className?: string
}
const RecycleBulkCSVUpload: React.FC<RecycleBulkCSVUploadProps> = ({
  ...
  return (
    <section className={clsx("rounded-lg shadow border p-6", cardBg, className)}>
      ...
      <p className="mt-2 text-xs text-gray-500">
        Download template CSV for required columns (Asset Type, Serial, Brand, Model, Condition, Address, etc).
      </p>
    </section>
  )
}
export default RecycleBulkCSVUpload
