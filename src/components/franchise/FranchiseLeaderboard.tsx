...
export interface FranchiseLeaderboardEntry {
  id: string
  name: string
  metric: string
  value: number
...
export interface FranchiseLeaderboardProps {
  entries: FranchiseLeaderboardEntry[]
  className?: string
}

const FranchiseLeaderboard: React.FC<FranchiseLeaderboardProps> = ({
  entries, className
}) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {entries.length === 0 ? (
              <tr>
                <td colSpan={5} className={clsx("text-center py-6", text)}>No data.</td>
              </tr>
            ) : (
              entries.map(entry => (
                <tr key={entry.id}>
